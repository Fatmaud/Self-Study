...
const getData = async () => {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`URL hatasi: ${response.status}`);
    }
    ...
    console.log(error);
    console.log("try-catch sayesinde kod valismaya devam eder");
  }
};
...
const ekranaBastir = (data) => {
  data.forEach((item) => {
    document.querySelector("section").innerHTML += `<h1 class="text-danger" >${
      item.show.name
    }</h1>
