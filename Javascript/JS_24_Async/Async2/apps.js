...
const getNews = async () => {
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error("News could not be fetched");
    }
...
const renderError = (err) => {
  document.querySelector("#news").innerHTML = `<h1>${err}</h1>
  <img src="./404.png"/>
  `;
};
...
const renderNews = (news) => {
  news.forEach((news) => {
    //!destructuring
    const { urlToImage, title, content, url } = news;
  ...
  <img src="${
    news.urlToImage ? news.urlToImage : defaultImage
